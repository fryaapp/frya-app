# P-41: Kunden-API Endpoints Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build 10 REST endpoints under `/api/v1/` that the customer UI (Web + Android) needs.

**Architecture:** Single file `agent/app/api/customer_api.py` with an APIRouter (prefix `/api/v1`, tag `customer`). All endpoints use `require_authenticated` for auth. Tenant resolved via `tenant_resolver.resolve_tenant_id()` (single-tenant MVP). The chat endpoint reuses TelegramCommunicatorService logic. Approval endpoint reuses BookingApprovalService.

**Tech Stack:** Python 3.11, FastAPI, asyncpg, Redis, Pydantic, Paperless-ngx API, Akaunting API

**Spec:** `C:\Users\lenovo\Downloads\DEV-V3-PAKET-P-41-Kunden-API.md`

---

## File Map

| File | Action | Purpose |
|------|--------|---------|
| `agent/app/api/customer_api.py` | Create | All 10 endpoints + Pydantic request/response models |
| `agent/app/main.py:246-279` | Modify | Register customer_router |
| `agent/app/connectors/dms_paperless.py` | Modify | Add `get_thumbnail_bytes()` method |
| `agent/tests/test_p41_customer_api.py` | Create | Tests for all endpoints |

---

### Task 1: Scaffold + Router Registration + Tenant Helper

**Files:**
- Create: `agent/app/api/customer_api.py`
- Modify: `agent/app/main.py`
- Test: `agent/tests/test_p41_customer_api.py`

- [ ] **Step 1: Write test**

```python
# agent/tests/test_p41_customer_api.py
"""Tests for P-41: Kunden-API Endpoints."""
import pytest


def test_customer_router_exists():
    """customer_api module exposes a router."""
    from app.api.customer_api import router
    assert router is not None
    assert router.prefix == '/api/v1'
```

- [ ] **Step 2: Create scaffold**

```python
# agent/app/api/customer_api.py
"""P-41: Customer-facing REST API endpoints.

All endpoints require session auth and are tenant-scoped.
"""
from __future__ import annotations

import logging
import uuid
from datetime import date, datetime, timezone
from decimal import Decimal
from typing import Any

from fastapi import APIRouter, Depends, HTTPException, UploadFile
from pydantic import BaseModel, Field

from app.auth.dependencies import require_authenticated
from app.auth.models import AuthUser

logger = logging.getLogger(__name__)

router = APIRouter(prefix='/api/v1', tags=['customer'])


async def _resolve_tenant_uuid() -> uuid.UUID:
    """Resolve the single-tenant UUID. Raises 503 if unavailable."""
    from app.case_engine.tenant_resolver import resolve_tenant_id
    tid = await resolve_tenant_id()
    if not tid:
        raise HTTPException(status_code=503, detail='tenant_unavailable')
    return uuid.UUID(tid)


def _load_translations() -> dict:
    """Load ui_translations.yaml (cached after first call)."""
    import yaml
    from pathlib import Path
    _path = Path(__file__).resolve().parent.parent.parent / 'data' / 'config' / 'ui_translations.yaml'
    with open(_path, encoding='utf-8') as f:
        return yaml.safe_load(f)


_TRANSLATIONS: dict | None = None


def _get_translations() -> dict:
    global _TRANSLATIONS
    if _TRANSLATIONS is None:
        _TRANSLATIONS = _load_translations()
    return _TRANSLATIONS
```

- [ ] **Step 3: Register router in main.py**

Add import near top of main.py (with other router imports):
```python
from app.api.customer_api import router as customer_router
```

Add registration at end (before the `@app.post('/agent/run')` line):
```python
app.include_router(customer_router)
```

- [ ] **Step 4: Run test, commit**

Run: `cd agent && python -m pytest tests/test_p41_customer_api.py -v --no-header`

```bash
git add agent/app/api/customer_api.py agent/app/main.py agent/tests/test_p41_customer_api.py
git commit -m "feat(P-41): scaffold customer_api.py + router registration"
```

---

### Task 2: POST /api/v1/chat

**Files:**
- Modify: `agent/app/api/customer_api.py`
- Test: `agent/tests/test_p41_customer_api.py`

The chat endpoint must reuse the Communicator pipeline without duplicating Telegram-specific code. It creates a synthetic `TelegramNormalizedIngressMessage` and calls `TelegramCommunicatorService.try_handle_turn()`.

- [ ] **Step 1: Write tests**

```python
@pytest.mark.asyncio
async def test_chat_endpoint_returns_reply():
    """POST /chat with valid message returns a reply."""
    from app.api.customer_api import router, ChatRequest
    # Test the Pydantic model
    req = ChatRequest(message='Was habe ich bei A&S gekauft?')
    assert req.message == 'Was habe ich bei A&S gekauft?'


def test_chat_request_model():
    from app.api.customer_api import ChatRequest, ChatResponse
    req = ChatRequest(message='Hallo')
    assert req.message == 'Hallo'
    resp = ChatResponse(reply='FRYA: Hallo!', case_ref=None, suggestions=[])
    assert resp.reply.startswith('FRYA:')
```

- [ ] **Step 2: Implement**

Add to `customer_api.py`:

```python
# ── Request/Response Models ──────────────────────────────────────────────────

class ChatRequest(BaseModel):
    message: str = Field(min_length=1, max_length=2000)


class ChatResponse(BaseModel):
    reply: str
    case_ref: str | None = None
    suggestions: list[str] = Field(default_factory=list)


# ── POST /chat ───────────────────────────────────────────────────────────────

@router.post('/chat')
async def send_chat_message(
    body: ChatRequest,
    user: AuthUser = Depends(require_authenticated),
) -> ChatResponse:
    """Send a message to Frya and get a synchronous reply."""
    from app.dependencies import (
        get_audit_service,
        get_case_repository,
        get_chat_history_store,
        get_communicator_conversation_store,
        get_communicator_user_store,
        get_open_items_service,
        get_telegram_clarification_service,
        get_telegram_communicator_service,
    )
    from app.telegram.communicator.intent_classifier import classify_intent
    from app.telegram.models import (
        TelegramActorInfo,
        TelegramNormalizedIngressMessage,
    )

    chat_id = f'web-{user.username}'
    case_id = f'web-chat-{user.username}-{uuid.uuid4().hex[:8]}'

    normalized = TelegramNormalizedIngressMessage(
        update_id=0,
        message_id=0,
        telegram_chat_ref=f'web:{user.username}',
        actor=TelegramActorInfo(
            chat_id=chat_id,
            sender_id=user.username,
            sender_username=user.username,
        ),
        text=body.message,
        media_attachments=[],
    )

    communicator = get_telegram_communicator_service()
    result = await communicator.try_handle_turn(
        normalized,
        case_id,
        audit_service=get_audit_service(),
        open_items_service=get_open_items_service(),
        clarification_service=get_telegram_clarification_service(),
        conversation_store=get_communicator_conversation_store(),
        user_store=get_communicator_user_store(),
        case_repository=get_case_repository(),
        chat_history_store=get_chat_history_store(),
    )

    if result is None or not result.reply_text:
        return ChatResponse(reply='FRYA: Ich konnte deine Nachricht nicht verarbeiten.', suggestions=[])

    # Extract case_ref from conversation memory
    conv_store = get_communicator_conversation_store()
    conv_mem = await conv_store.load(chat_id)
    case_ref = conv_mem.last_case_ref if conv_mem else None

    # Context-dependent suggestion chips
    intent = classify_intent(body.message)
    suggestions = _build_suggestions(intent, case_ref)

    return ChatResponse(
        reply=result.reply_text,
        case_ref=case_ref,
        suggestions=suggestions,
    )


def _build_suggestions(intent: str | None, case_ref: str | None) -> list[str]:
    if case_ref:
        return ['Details anzeigen', 'Buchen', 'Rechnung suchen']
    if intent == 'GREETING':
        return ['Status-Übersicht', 'Offene Belege', 'Frist-Check']
    return ['Offene Belege', 'Rechnung suchen']
```

- [ ] **Step 3: Run tests, commit**

```bash
git commit -m "feat(P-41): POST /api/v1/chat endpoint"
```

---

### Task 3: GET /api/v1/inbox + POST /inbox/{case_id}/approve

**Files:**
- Modify: `agent/app/api/customer_api.py`
- Test: `agent/tests/test_p41_customer_api.py`

- [ ] **Step 1: Write tests**

```python
def test_inbox_item_model():
    from app.api.customer_api import InboxItem
    item = InboxItem(
        case_id='uuid-123', case_number='CASE-001', vendor_name='Test GmbH',
        amount=100.0, currency='EUR', document_type='Eingangsrechnung',
        status='OPEN', created_at='2026-03-23T20:00:00Z',
    )
    assert item.vendor_name == 'Test GmbH'


def test_approval_request_model():
    from app.api.customer_api import ApprovalRequest
    req = ApprovalRequest(action='approve')
    assert req.action == 'approve'
    # Invalid action
    import pydantic
    with pytest.raises(pydantic.ValidationError):
        ApprovalRequest(action='invalid')
```

- [ ] **Step 2: Implement**

Add to `customer_api.py`:

```python
# ── Inbox Models ─────────────────────────────────────────────────────────────

class InboxItem(BaseModel):
    case_id: str
    case_number: str | None = None
    vendor_name: str | None = None
    amount: float | None = None
    currency: str = 'EUR'
    document_type: str | None = None
    status: str | None = None
    approval_mode: str | None = None
    confidence: float | None = None
    confidence_label: str | None = None
    created_at: str | None = None
    due_date: str | None = None
    risk_flags: list[str] = Field(default_factory=list)
    booking_proposal: dict | None = None


class InboxResponse(BaseModel):
    count: int
    items: list[InboxItem]


class ApprovalRequest(BaseModel):
    action: str = Field(pattern=r'^(approve|correct|reject|defer)$')
    corrections: dict | None = None


# ── GET /inbox ───────────────────────────────────────────────────────────────

@router.get('/inbox')
async def get_inbox(
    user: AuthUser = Depends(require_authenticated),
    status: str = 'pending',
    limit: int = 50,
    offset: int = 0,
) -> InboxResponse:
    """Return cases awaiting user action."""
    from app.dependencies import get_case_repository
    tenant_id = await _resolve_tenant_uuid()
    repo = get_case_repository()

    all_cases = await repo.list_active_cases_for_tenant(tenant_id)
    # Include DRAFT cases for inbox
    try:
        drafts = await repo.list_cases_by_status(tenant_id, 'DRAFT')
        seen = {c.id for c in all_cases}
        for d in drafts:
            if d.id not in seen:
                all_cases.append(d)
    except Exception:
        pass

    if status == 'pending':
        all_cases = [c for c in all_cases if c.status in ('DRAFT', 'OPEN')]

    # Sort by created_at desc
    all_cases.sort(key=lambda c: c.created_at or datetime.min, reverse=True)
    page = all_cases[offset:offset + limit]

    tr = _get_translations()
    items = []
    for c in page:
        meta = c.metadata or {}
        bp = meta.get('booking_proposal')
        conf = c.metadata.get('overall_confidence') if c.metadata else None
        items.append(InboxItem(
            case_id=str(c.id),
            case_number=c.case_number,
            vendor_name=c.vendor_name,
            amount=float(c.total_amount) if c.total_amount else None,
            currency=c.currency or 'EUR',
            document_type=tr.get('document_type', {}).get(
                meta.get('document_analysis', {}).get('document_type', ''), 'Sonstiges'),
            status=c.status,
            confidence=conf,
            confidence_label=_confidence_label(conf, tr),
            created_at=c.created_at.isoformat() if c.created_at else None,
            due_date=str(c.due_date) if c.due_date else None,
            booking_proposal=bp,
        ))

    return InboxResponse(count=len(all_cases), items=items)


def _confidence_label(conf: float | None, tr: dict) -> str | None:
    if conf is None:
        return None
    labels = tr.get('confidence', {})
    if conf >= 0.85:
        return labels.get('CERTAIN', 'Sicher')
    if conf >= 0.65:
        return labels.get('HIGH', 'Hoch')
    if conf >= 0.40:
        return labels.get('MEDIUM', 'Mittel')
    return labels.get('LOW', 'Niedrig')


# ── POST /inbox/{case_id}/approve ────────────────────────────────────────────

@router.post('/inbox/{case_id}/approve')
async def approve_inbox_item(
    case_id: str,
    body: ApprovalRequest,
    user: AuthUser = Depends(require_authenticated),
) -> dict:
    """Process an approval action for a case."""
    from app.booking.approval_service import BookingApprovalService
    from app.dependencies import (
        get_akaunting_connector,
        get_approval_service,
        get_audit_service,
        get_open_items_service,
    )

    approval_svc = get_approval_service()
    pending = [
        r for r in await approval_svc.list_by_case(case_id)
        if r.status == 'PENDING' and r.action_type == 'booking_finalize'
    ]
    if not pending:
        raise HTTPException(status_code=404, detail='no_pending_approval')

    booking_svc = BookingApprovalService(
        approval_service=approval_svc,
        open_items_service=get_open_items_service(),
        audit_service=get_audit_service(),
        akaunting_connector=get_akaunting_connector(),
    )

    result = await booking_svc.process_response(
        case_id=case_id,
        approval_id=pending[0].approval_id,
        decision_raw=body.action.upper(),
        decided_by=user.username,
        correction_payload=body.corrections,
        source='customer_api',
    )

    return {'status': 'processed', 'result': result}
```

- [ ] **Step 3: Run tests, commit**

```bash
git commit -m "feat(P-41): GET /inbox + POST /inbox/{id}/approve"
```

---

### Task 4: POST /inbox/{case_id}/learn

**Files:**
- Modify: `agent/app/api/customer_api.py`
- Test: `agent/tests/test_p41_customer_api.py`

- [ ] **Step 1: Write test**

```python
def test_learn_request_model():
    from app.api.customer_api import LearnRequest
    req = LearnRequest(scope='vendor_always', rule='ARAL immer privat')
    assert req.scope == 'vendor_always'
```

- [ ] **Step 2: Implement**

```python
class LearnRequest(BaseModel):
    scope: str = Field(pattern=r'^(this_only|vendor_always|category_always|ask_every_time)$')
    rule: str | None = None


@router.post('/inbox/{case_id}/learn')
async def learn_from_correction(
    case_id: str,
    body: LearnRequest,
    user: AuthUser = Depends(require_authenticated),
) -> dict:
    """Store a learning effect for future assignments."""
    from app.dependencies import get_audit_service, get_case_repository

    audit = get_audit_service()
    await audit.log_event({
        'event_id': str(uuid.uuid4()),
        'case_id': case_id,
        'source': 'customer_api',
        'agent_name': 'memory-curator',
        'approval_status': 'NOT_REQUIRED',
        'action': 'LEARN_RULE_SUBMITTED',
        'result': f'scope={body.scope}, rule={body.rule or "-"}',
    })

    # For vendor_always: store in case metadata as hint for future matching
    if body.scope == 'vendor_always' and body.rule:
        try:
            repo = get_case_repository()
            case = await repo.get_case(uuid.UUID(case_id))
            if case:
                await repo.update_metadata(case.id, {
                    'learn_rule': {'scope': body.scope, 'rule': body.rule, 'by': user.username}
                })
        except Exception as exc:
            logger.warning('Learn rule save failed for %s: %s', case_id, exc)

    return {'status': 'accepted', 'scope': body.scope}
```

- [ ] **Step 3: Run tests, commit**

```bash
git commit -m "feat(P-41): POST /inbox/{id}/learn"
```

---

### Task 5: GET /api/v1/documents + POST /documents/upload + GET /documents/{id}/thumbnail

**Files:**
- Modify: `agent/app/api/customer_api.py`
- Modify: `agent/app/connectors/dms_paperless.py`
- Test: `agent/tests/test_p41_customer_api.py`

- [ ] **Step 1: Write tests**

```python
def test_document_item_model():
    from app.api.customer_api import DocumentItem
    item = DocumentItem(id=16, title='Test', tags=['frya:analysiert'])
    assert item.id == 16


@pytest.mark.asyncio
async def test_paperless_get_thumbnail_bytes():
    """PaperlessConnector has get_thumbnail_bytes method."""
    from app.connectors.dms_paperless import PaperlessConnector
    assert hasattr(PaperlessConnector, 'get_thumbnail_bytes')
```

- [ ] **Step 2: Add get_thumbnail_bytes to PaperlessConnector**

In `agent/app/connectors/dms_paperless.py`, after `download_document_bytes`:

```python
    async def get_thumbnail_bytes(self, doc_id: str) -> bytes:
        """Download the thumbnail image for a Paperless document.

        GET /api/documents/{id}/thumb/
        Returns: raw image bytes (PNG/JPEG).
        """
        async with httpx.AsyncClient(timeout=20) as client:
            response = await client.get(
                f'{self.base_url}/api/documents/{doc_id}/thumb/',
                headers=self._headers(),
            )
            response.raise_for_status()
            return response.content
```

- [ ] **Step 3: Implement document endpoints**

```python
class DocumentItem(BaseModel):
    id: int
    title: str | None = None
    correspondent: str | None = None
    document_type: str | None = None
    tags: list[str] = Field(default_factory=list)
    created_at: str | None = None
    thumbnail_url: str | None = None


class DocumentsResponse(BaseModel):
    count: int
    items: list[DocumentItem]


@router.get('/documents')
async def search_documents(
    user: AuthUser = Depends(require_authenticated),
    query: str = '',
    document_type: str = '',
    limit: int = 25,
    offset: int = 0,
) -> DocumentsResponse:
    """Search documents in Paperless."""
    from app.dependencies import get_paperless_connector
    pc = get_paperless_connector()

    search_query = query
    if document_type:
        search_query = f'{search_query} type:{document_type}'.strip()

    docs = await pc.search_documents(search_query) if search_query else await pc.list_all_documents()
    if isinstance(docs, list):
        total = len(docs)
        page = docs[offset:offset + limit]
    else:
        total = 0
        page = []

    items = []
    for d in page:
        items.append(DocumentItem(
            id=d.get('id', 0),
            title=d.get('title'),
            correspondent=d.get('correspondent_name') or d.get('correspondent'),
            document_type=d.get('document_type_name') or d.get('document_type'),
            tags=[t if isinstance(t, str) else str(t) for t in (d.get('tags') or [])],
            created_at=d.get('created') or d.get('added'),
            thumbnail_url=f'/api/v1/documents/{d.get("id", 0)}/thumbnail',
        ))

    return DocumentsResponse(count=total, items=items)


@router.post('/documents/upload')
async def upload_document(
    file: UploadFile,
    user: AuthUser = Depends(require_authenticated),
) -> dict:
    """Upload a document and start the analysis pipeline."""
    from app.dependencies import get_paperless_connector

    content = await file.read()
    if len(content) > 20_000_000:
        raise HTTPException(status_code=400, detail='file_too_large')

    pc = get_paperless_connector()
    ref = f'web-upload-{uuid.uuid4().hex[:8]}'
    result = await pc.upload_document(
        content,
        filename=file.filename or 'upload.pdf',
        title=f'frya:{ref}:{file.filename or "upload"}',
    )

    return {
        'ref': ref,
        'status': 'processing',
        'message': 'Dokument angenommen. Analyse läuft.',
        'task_id': result.get('task_id'),
    }


@router.get('/documents/{doc_id}/thumbnail')
async def get_document_thumbnail(
    doc_id: int,
    user: AuthUser = Depends(require_authenticated),
):
    """Proxy to Paperless document thumbnail."""
    from fastapi.responses import Response
    from app.dependencies import get_paperless_connector

    pc = get_paperless_connector()
    try:
        thumb_bytes = await pc.get_thumbnail_bytes(str(doc_id))
    except Exception:
        raise HTTPException(status_code=404, detail='thumbnail_not_found')

    return Response(content=thumb_bytes, media_type='image/png')
```

- [ ] **Step 4: Run tests, commit**

```bash
git commit -m "feat(P-41): documents search + upload + thumbnail endpoints"
```

---

### Task 6: GET /api/v1/cases + GET /cases/{id}

**Files:**
- Modify: `agent/app/api/customer_api.py`
- Test: `agent/tests/test_p41_customer_api.py`

- [ ] **Step 1: Write tests**

```python
def test_case_detail_model():
    from app.api.customer_api import CaseDetail
    detail = CaseDetail(
        case_id='uuid', case_number='CASE-001', vendor_name='Test',
        status='OPEN',
    )
    assert detail.status == 'OPEN'
```

- [ ] **Step 2: Implement**

```python
class CaseDetail(BaseModel):
    case_id: str
    case_number: str | None = None
    vendor_name: str | None = None
    amount: float | None = None
    currency: str = 'EUR'
    status: str | None = None
    case_type: str | None = None
    created_at: str | None = None
    due_date: str | None = None
    document_analysis: dict | None = None
    booking_proposal: dict | None = None
    line_items: list[dict] = Field(default_factory=list)
    timeline: list[dict] = Field(default_factory=list)


class CasesResponse(BaseModel):
    count: int
    items: list[CaseDetail]


@router.get('/cases')
async def list_cases(
    user: AuthUser = Depends(require_authenticated),
    status: str = '',
    limit: int = 50,
    offset: int = 0,
) -> CasesResponse:
    """List all cases for the tenant."""
    from app.dependencies import get_case_repository
    tenant_id = await _resolve_tenant_uuid()
    repo = get_case_repository()

    if status:
        cases = await repo.list_cases_by_status(tenant_id, status.upper())
    else:
        # All non-deleted cases
        all_statuses = ['DRAFT', 'OPEN', 'OVERDUE', 'PAID', 'CLOSED']
        cases = []
        for s in all_statuses:
            try:
                cases.extend(await repo.list_cases_by_status(tenant_id, s))
            except Exception:
                pass

    cases.sort(key=lambda c: c.created_at or datetime.min, reverse=True)
    total = len(cases)
    page = cases[offset:offset + limit]

    items = [_case_to_detail(c) for c in page]
    return CasesResponse(count=total, items=items)


@router.get('/cases/{case_id}')
async def get_case_detail(
    case_id: str,
    user: AuthUser = Depends(require_authenticated),
) -> CaseDetail:
    """Get full case details including line items and timeline."""
    from app.dependencies import get_audit_service, get_case_repository
    tenant_id = await _resolve_tenant_uuid()
    repo = get_case_repository()

    case = await repo.get_case(uuid.UUID(case_id))
    if case is None or case.tenant_id != tenant_id:
        raise HTTPException(status_code=404, detail='case_not_found')

    detail = _case_to_detail(case)

    # Add timeline from audit events
    try:
        audit = get_audit_service()
        # Find orchestration case_id via audit trail
        events = await audit.by_case(str(case.id), limit=50)
        if not events:
            # Try finding via case_number or any audit reference
            for _prefix in ['doc-', 'tg-']:
                refs = await audit.by_case(f'{_prefix}{case.case_number}', limit=50)
                if refs:
                    events = refs
                    break
        detail.timeline = [
            {
                'action': getattr(ev, 'action', ''),
                'result': str(getattr(ev, 'result', ''))[:100],
                'agent': getattr(ev, 'agent_name', ''),
                'created_at': str(getattr(ev, 'created_at', ''))[:19],
            }
            for ev in (events or [])
        ]
    except Exception as exc:
        logger.warning('Timeline fetch failed for case %s: %s', case_id, exc)

    return detail


def _case_to_detail(case: Any) -> CaseDetail:
    meta = case.metadata or {}
    da = meta.get('document_analysis', {})
    return CaseDetail(
        case_id=str(case.id),
        case_number=case.case_number,
        vendor_name=case.vendor_name,
        amount=float(case.total_amount) if case.total_amount else None,
        currency=case.currency or 'EUR',
        status=case.status,
        case_type=case.case_type if hasattr(case, 'case_type') else None,
        created_at=case.created_at.isoformat() if case.created_at else None,
        due_date=str(case.due_date) if case.due_date else None,
        document_analysis=da or None,
        booking_proposal=meta.get('booking_proposal'),
        line_items=da.get('line_items', []),
    )
```

- [ ] **Step 3: Run tests, commit**

```bash
git commit -m "feat(P-41): GET /cases + GET /cases/{id}"
```

---

### Task 7: GET /api/v1/deadlines

**Files:**
- Modify: `agent/app/api/customer_api.py`
- Test: `agent/tests/test_p41_customer_api.py`

- [ ] **Step 1: Write test**

```python
def test_deadlines_response_model():
    from app.api.customer_api import DeadlinesResponse
    resp = DeadlinesResponse(
        overdue=[], due_today=[], due_soon=[], skonto_expiring=[],
        summary='Keine überfälligen Vorgänge.',
    )
    assert 'Keine' in resp.summary
```

- [ ] **Step 2: Implement**

```python
class DeadlinesResponse(BaseModel):
    overdue: list[dict] = Field(default_factory=list)
    due_today: list[dict] = Field(default_factory=list)
    due_soon: list[dict] = Field(default_factory=list)
    skonto_expiring: list[dict] = Field(default_factory=list)
    summary: str = ''


@router.get('/deadlines')
async def get_deadlines(
    user: AuthUser = Depends(require_authenticated),
) -> DeadlinesResponse:
    """Return deadline overview — overdue, due today, due soon, skonto expiring."""
    from app.deadline_analyst.service import DeadlineAnalystService
    from app.dependencies import get_case_repository

    tenant_id = await _resolve_tenant_uuid()
    repo = get_case_repository()
    svc = DeadlineAnalystService(repo)

    report = await svc.check_all_deadlines(tenant_id)

    def _case_dict(item: Any) -> dict:
        return {
            'case_id': str(getattr(item, 'case_id', '')),
            'case_number': getattr(item, 'case_number', None),
            'vendor_name': getattr(item, 'vendor_name', None),
            'amount': float(getattr(item, 'amount', 0)) if getattr(item, 'amount', None) else None,
            'due_date': str(getattr(item, 'due_date', '')),
            'days_remaining': getattr(item, 'days_remaining', None),
            'severity': getattr(item, 'severity', None),
        }

    return DeadlinesResponse(
        overdue=[_case_dict(c) for c in (report.overdue or [])],
        due_today=[_case_dict(c) for c in (report.due_today or [])],
        due_soon=[_case_dict(c) for c in (report.due_soon or [])],
        skonto_expiring=[_case_dict(c) for c in (report.skonto_expiring or [])],
        summary=report.summary or '',
    )
```

- [ ] **Step 3: Run tests, commit**

```bash
git commit -m "feat(P-41): GET /deadlines"
```

---

### Task 8: GET /api/v1/finance/summary

**Files:**
- Modify: `agent/app/api/customer_api.py`
- Test: `agent/tests/test_p41_customer_api.py`

- [ ] **Step 1: Write test**

```python
def test_finance_summary_model():
    from app.api.customer_api import FinanceSummaryResponse
    resp = FinanceSummaryResponse(
        period='März 2026', income=4200.0, expenses=1850.0,
        open_receivables=1200.0, open_payables=650.0,
    )
    assert resp.income == 4200.0
```

- [ ] **Step 2: Implement**

```python
class FinanceSummaryResponse(BaseModel):
    period: str = ''
    income: float = 0.0
    expenses: float = 0.0
    open_receivables: float = 0.0
    open_payables: float = 0.0
    overdue_count: int = 0
    overdue_amount: float = 0.0


@router.get('/finance/summary')
async def get_finance_summary(
    user: AuthUser = Depends(require_authenticated),
    period: str = 'month',
) -> FinanceSummaryResponse:
    """Return financial summary — income, expenses, open items."""
    from app.dependencies import get_akaunting_connector

    ak = get_akaunting_connector()
    now = datetime.now(timezone.utc)

    if period == 'quarter':
        q_start_month = ((now.month - 1) // 3) * 3 + 1
        months = list(range(q_start_month, now.month + 1))
        period_label = f'Q{(now.month - 1) // 3 + 1} {now.year}'
    elif period == 'year':
        months = list(range(1, now.month + 1))
        period_label = str(now.year)
    else:
        months = [now.month]
        period_label = now.strftime('%B %Y')

    total_income = 0.0
    total_expenses = 0.0
    for m in months:
        try:
            summary = await ak.get_monthly_summary(now.year, m)
            total_income += summary.get('total_income', 0.0)
            total_expenses += summary.get('total_expense', 0.0)
        except Exception as exc:
            logger.warning('Finance summary fetch failed for %d-%02d: %s', now.year, m, exc)

    # Open items
    try:
        open_items = await ak.get_open_items_summary()
        open_recv = open_items.get('total_receivable', 0.0)
        open_pay = open_items.get('total_payable', 0.0)
    except Exception as exc:
        logger.warning('Open items fetch failed: %s', exc)
        open_recv = 0.0
        open_pay = 0.0

    # Overdue count from case engine
    overdue_count = 0
    overdue_amount = 0.0
    try:
        from app.dependencies import get_case_repository
        tenant_id = await _resolve_tenant_uuid()
        repo = get_case_repository()
        overdue_cases = await repo.list_cases_by_status(tenant_id, 'OVERDUE')
        overdue_count = len(overdue_cases)
        overdue_amount = sum(float(c.total_amount or 0) for c in overdue_cases)
    except Exception:
        pass

    return FinanceSummaryResponse(
        period=period_label,
        income=total_income,
        expenses=total_expenses,
        open_receivables=open_recv,
        open_payables=open_pay,
        overdue_count=overdue_count,
        overdue_amount=overdue_amount,
    )
```

- [ ] **Step 3: Run tests, commit**

```bash
git commit -m "feat(P-41): GET /finance/summary"
```

---

### Task 9: Full test suite + final commit

- [ ] **Step 1: Run full test suite**

```bash
cd agent && python -m pytest tests/ -v --no-header --tb=short -q 2>&1 | tail -5
```

- [ ] **Step 2: Fix any failures**

- [ ] **Step 3: Final commit**

```bash
git commit -m "feat(P-41): 10 Kunden-API Endpoints unter /api/v1/"
```

---

## Verification Checklist

After deploy, verify (401 is expected for auth-protected endpoints):

1. `wc -l customer_api.py` → substantial file
2. `grep customer_router main.py` → registered
3. `curl /api/v1/chat` → 401 or 422
4. `curl /api/v1/inbox` → 401
5. `curl /api/v1/cases` → 401
6. `curl /api/v1/deadlines` → 401
7. `curl /api/v1/finance/summary` → 401
8. Health → 200
